...
def intent_detector(state: AgentState) -> AgentState:
    ...
    # IMPROVED classification prompt - more context-aware and includes external search
    prompt = f"""
    You are an advanced, low-temperature, deterministic **INTENT CLASSIFIER** for an agent workflow.
    Your **ONLY** task is to analyze the user's request and the provided Context, and return a JSON object indicating the next required step (action).
    
    DO NOT generate any conversational text, explanations, or answers outside of the JSON object.
    
    Return EXACTLY one JSON object with one of these forms:

    1) If the question is purely about **general knowledge** (e.g., "what is love?", "what is the capital of France").
    ...
    3) If the question references "this document", "the file", "the PDF", or can be **fully and comprehensively answered** using **ONLY** the provided Context:
    {{"action": "answer", "answer": "<full answer text based on context>"}}

    4) If the question **requires both** the document Context **AND** external, up-to-date information (e.g., "Compare the Q3 revenue in this document with current market trends") OR if it is a complex request requiring multi-step reasoning.
    {{"action": "agentic", "reasoning": "needs document context and/or external search"}}

    5) If the question **requires** real time external data (e.g., what is the current stock price of tesla, who won the womens world cup)
    {{"action": "agentic", "reasoning": "needs to use the web search tool"}}

    IMPORTANT CLASSIFICATION RULES:
    - If the query mentions document terms ("this document," "PDF," "file") and needs any external, non-document knowledge → use "agentic" (Action 4).
    - If the user asks for general knowledge and it is **not** related to the context → **ALWAYS use "general_query"**.
    - If the user asks about something which requires pure real time data → use **agentic** (Action 5).
    - **NEVER** include an explanation, apology, or preamble outside the JSON object.
